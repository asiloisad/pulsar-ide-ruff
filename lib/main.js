...
class RuffLanguageClient extends AutoLanguageClient {
  ...
  activate() {
    ...
    const old = atom.config.get(`${PACKAGE_NAME}.ruffExecutable`);
    if (old != null) {
      atom.config.set(`${PACKAGE_NAME}.ruffCommand`, old);
      atom.config.unset(`${PACKAGE_NAME}.ruffExecutable`);
    }

    // Register commands
    this.commandSubscription = atom.commands.add("atom-workspace", {
      "ide-ruff:restart-server": () => this.restartServer(),
      "ide-ruff:lint-projects": () => indie.runScan(),
      "ide-ruff:lint-selected": () => indie.runSelectedScan(),
      "ide-ruff:toggle-noqa": () => this.toggleNoqa(),
      "ide-ruff:global-pyproject": () => this.openGlobalConfig(),
    });
    this.treeViewCommandSubscription = atom.commands.add(".tree-view", {
      "ide-ruff:lint-selected": () => indie.runSelectedScan(),
    });
  ...
  preInitialization(connection) {
    ...
    connection.onPublishDiagnostics = (callback) => {
      return originalOn((params) => {
        ...
        const delay = Number(atom.config.get(`${PACKAGE_NAME}.diagnosticDebounceMs`) || 0);
        if (delay <= 0) {
          callback(params);
          return;
        }

        const key = params.uri || "__global__";
        const existingTimer = this.diagnosticTimers.get(key);
        if (existingTimer) {
  ...
  consumeLinterV2(registerIndie) {
    ...
    const projectDelegate = registerIndie({
      name: "Ruff/Project",
      deleteOnOpen: atom.config.get(`${PACKAGE_NAME}.deleteOnOpen`),
    });
  ...
  consumeBusySignal(busySignal) {
    if (typeof super.consumeBusySignal === "function") {
      super.consumeBusySignal(busySignal);
    }
  ...
  startServerProcess(projectPath) {
    const command = atom.config.get(`${PACKAGE_NAME}.ruffCommand`) || "ruff";
    const [exe, ...extraArgs] = command.trim().split(/\s+/);

    const exePath = whichSync(exe, { nothrow: true });
    if (!exePath) {
      log(`Executable not found: ${exe}`);
      this.ruffCommand = command;
      this.ruffFound = false;
      const err = new Error(`ruff command not found: ${command}`);
      err.code = "ENOENT";
      throw err;
    }

    log(`Project: ${projectPath}`);
    log(`Ruff command: ${exePath} ${[...extraArgs, "server"].join(" ")}`);

    this.ruffCommand = command;
    this.ruffFound = true;
    indie.setRuffPath(exePath, extraArgs);

    const childProcess = cp.spawn(exePath, [...extraArgs, "server"], {
      cwd: projectPath,
    });
  ...
  onSpawnError(err) {
    ...
    const description =
      err.code === "ENOENT"
        ? `Command not found: \`${this.ruffCommand}\`.`
        : `Could not start ruff: \`${this.ruffCommand}\`.`;
  ...
  async deactivate() {
    ...
    if (this.treeViewCommandSubscription) {
      this.treeViewCommandSubscription.dispose();
    }
    ...
    try {
      await Promise.race([super.deactivate(), new Promise((resolve) => setTimeout(resolve, 2000))]);
    } catch (err) {
